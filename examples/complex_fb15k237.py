...
from torchkge import LinkPredictionEvaluator, TripletClassificationEvaluator
from torchkge.models import ComplExModel
from torchkge.sampling import TuckerNegativeSampling
from torchkge.utils import BinaryCrossEntropyLoss, DataLoader, load_fb15k237
...
def main():
    ...
    sampler = TuckerNegativeSampling(kg=kg_train, kg_model=model, kg_val=None, kg_test=kg_test, cache_dim=cache_dim,
                                     n_itter=100, k=2, growth=0.2, n_factors=50, n_neg=n_neg)
    optimizer = Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    dataloader = DataLoader(kg_train, batch_size=b_size, use_cuda=use_cuda)
    evaluator = LinkPredictionEvaluator(model, kg_test)
    comp_eval = TripletClassificationEvaluator(model, kg_val, kg_test)
    ...
    for epoch in iterator:
        ...
        if epoch % 25 == 0 and epoch != 0:
            ...
            comp_eval.evaluate(10)
            comp_acc = comp_eval.accuracy(10)

            print("Completion accuracy: " + str(comp_acc))
